...
class Reload(object):
    ...
    def _update(self, namespace, name, oldobj, newobj,
        ...
        try:
            ...
            if isinstance(newobj, types.FunctionType):
                ...
                self._update_function(oldobj, newobj, for_class)
                return

            if isinstance(newobj, types.MethodType):
                notify_info2('Update method: ', oldobj, newobj)
                self._update_method(oldobj, newobj, for_class)
                return

            if isinstance(newobj, classmethod):
                notify_info2('Update classmethod: ', oldobj, newobj)
                self._update_classmethod(oldobj, newobj, for_class)
                return

            if isinstance(newobj, staticmethod):
                notify_info2('Update staticmethod: ', oldobj, newobj)
                self._update_staticmethod(oldobj, newobj, for_class)
                return
            ...
            if hasattr(newobj, '__call__') and hasattr(newobj, "__dict__"):
                notify_info2('Update callable: ', oldobj, newobj)
                self._update_callable(oldobj, newobj, for_class)
                return
    ...
    def _update_function(self, oldfunc, newfunc, for_class=None):
        ...
        for old_cell, new_cell in izip(old_closure, new_closure):
            if old_cell.cell_contents != for_class:
                self._update(None, None, old_cell.cell_contents, new_cell.cell_contents)

        return oldfunc


    def _update_method(self, oldmeth, newmeth, for_class=None):
        """Update a method object."""
        # XXX What if im_func is not a function?
        if hasattr(oldmeth, 'im_func') and hasattr(newmeth, 'im_func'):
            self._update(None, None, oldmeth.im_func, newmeth.im_func, for_class=for_class)
        elif hasattr(oldmeth, '__func__') and hasattr(newmeth, '__func__'):
            self._update(None, None, oldmeth.__func__, newmeth.__func__, for_class=for_class)
        return oldmeth
    ...
    def _update_class(self, oldclass, newclass):
        ...
        for name in (oldnames & newnames) - {'__dict__', '__doc__', "_gorg"}:
            self._update(oldclass, name, olddict[name], newdict[name], is_class_namespace=True, for_class=oldclass)
    ...
    def _update_classmethod(self, oldcm, newcm, for_class):
        ...
        self._update(None, None, oldcm.__get__(0), newcm.__get__(0), for_class=for_class)


    def _update_staticmethod(self, oldsm, newsm, for_class):
        ...
        self._update(None, None, oldsm.__get__(0), newsm.__get__(0), for_class=for_class)


    def _update_callable(self, oldcb, newcb, for_class):
        """
        ...
        for name in (oldnames & newnames) - {'__dict__', '__doc__'}:
            self._update(oldcb, name, olddict[name], newdict[name], for_class=for_class)
